...
Last modified: 2021-06-12 12:03:12
Filename     : orm.py
Description  : 处理DB表数据的ORM框架，映射DB表模型为类，支持查询、过滤等sql操作
"""
    ...
    def __new__(cls, cls_name, cls_bases, cls_attrs):
        ...
        for k, v in sorted(cls_attrs.items()):
            if isinstance(v, BaseField):
                ...
                # 保存属性和列的映射关系
                attr_map[k] = v
                # 取类属性对应的Field子类处理后的值，经过DB提取器处理之后的字段
                db_fields.append(v.db_field)
...
class Model(dict):
    ...
    def _make_where_sql(cls, options=SqlConfig.OPTIONS_AND, where=None):
        ...
        :param  where
            ...
            - 查询参数够着得有点奇葩，但也是无奈之举，DB有一个叫过滤器的东西，需要对查询的字段做一番转换
        ...
        for item in where:
            if not issubclass(item['filter'], BaseFilter):
                raise dbModelFilterNotFound(_filter=item['filter'])
            tmp_where.append('(' + item['filter']
    ...
    def _make_query_sql(cls, params):
        ...
        # 拼接 where 语句
        if params.get('where'):
            params['sql'] += cls._make_where_sql(params['options'],
                                                 params['where'])
            params.pop('where')

        # 拼接 order by 语句
        if params.get(SqlConfig.ORDER_BY):
            params['sql'] += ' %s ' % SqlConfig.ORDER_BY + params[
                SqlConfig.ORDER_BY]

        # 拼接 group by 语句
        if params.get('sort'):
            params['sql'] = " ".join([params['sql'], params['sort']])

        if params.get(SqlConfig.GROUP_BY):
            params['sql'] += ' %s ' % SqlConfig.GROUP_BY + params[
                SqlConfig.GROUP_BY]
    ...
    def all(cls):
        ...
        if SqlConfig.LIMIT not in cls.query_param:
            cls.query_param.update({SqlConfig.LIMIT: dbConfig.MAX_LIMIT})
        query_params = cls._make_query_sql(cls.query_param)
        query_ret = dbUtils.do_query(query_params)
    ...
    def count(cls):
        ...
        cls.query_param.update(
            {"sql": cls.__count__ + cls._make_query_table()})
        query_params = cls._make_query_sql(cls.query_param)
        query_ret = dbUtils.do_query(query_params)
    ...
    def sum(cls, field):
        ...
        if field not in cls.__mappings__:
            raise dbModelFieldsNotFound(cls.__table__, field)
    ...
    def asc(cls):
        ...
        :desc
            XXX 正序查询 从小到大
            DB 仅支持 select f1, blob_pos from 20180806:aci order by f1 limit 15
                这种对指定数量的数据做排序的查询，无法真正做到全表排序
                这样会存在一个问题，比如查询18条数据，那么后面3条数据顺序是未知的
            XXX 这种在order by 后添加 asc 的方法DB貌似支持
            文档路径： 
        """
        # 必须先order by再进行排序
        if SqlConfig.ORDER_BY not in cls.query_param:
            raise dbQueryParamErr()
    ...
    def _make_query_table(cls, date_range=False):
        ...
        :param tb_date
            查询日期
        :return query_table
            XXX 没有找到文档，查看DB文档的例子发现的用法
            指定日期，则返回 'blob:table:test_tb:20200717:test_tb'
            指定日期范围，则返回 '`blob/table/test_tb/[0-9]*/test_tb,20200710-20200718`'
            不指定日期，则返回 '`blob/table/test_tb/[0-9]*/test_tb`'
        """
        if cls.tb_date:
            query_table = ":".join(cls.__table__.split('/')) % str(cls.tb_date)
            return query_table
        else:
            query_table = "".join([
                dbConfig.MAGIC_CHAR,
                cls.__table__ % dbConfig.TABLE_REG,
                "/%s",  # 留个位置来填查询的日期区间
                dbConfig.MAGIC_CHAR
            ])
            return query_table % (',' + '-'.join(
                [str(cls.start_date), str(cls.end_date)])
                                  if date_range else "")
    ...
    def query(cls, start_date=None, end_date=None, tb_date=None):
        ...
        :param tb_date
            DB把数据按照上报日期存放到日期文件夹中，每个文件夹都认为是一张表
        ...
        cls.__select__ = cls.__select__ % {"collector": collector}
        # query_param 不要放在元类中
        # 否则上次查询的参数会留在类里面，影响下次查询
        cls.query_param = copy.deepcopy(dbConfig.BASE_QUERY_PARAM)
        cls.query_param.update(
    ...
    def filter(cls, where=None, options=SqlConfig.OPTIONS_AND):
        ...
        if where is not None:
            ...
            if delta_fields:
                raise dbModelFieldsNotFound(cls.__table__, delta_fields)
    ...
    def group_by(cls, *fields):
        ...
        cls.query_param.update(
            {SqlConfig.GROUP_BY: " %s " * field_cnt % fields})
        return cls
    ...
    def _offset_query(cls, mdd_path, page_num, page_size):
        ...
        cls.query_param.update({
            'op': dbConfig.QUERY_OP_LIST,
            'path': mdd_path,
            'begin': (page_num - 1) * page_size + 1,
            'end': page_num * page_size
        })
        query_ret = dbUtils.do_query(query_param=cls.query_param,
                                      clear_cache=False)
        return query_ret
    ...
    def offset(cls, page_num, mdd_path=None):
        ...
        e.g.:
            查询第3页的数据，每页大小为 1000
            test_tb.query().limit(1000).offset(3)
        """
        if SqlConfig.LIMIT not in cls.query_param:
            raise dbQueryParamErr()
        ...
        # 指定 limit，否则 db 默认只能查回 1000 条数据
        cls.query_param.update({SqlConfig.LIMIT: dbConfig.MAX_LIMIT})
        ...
        if not mdd_path:
            ...
            mdd_path = dbUtils.do_query(query_param=cls._make_query_sql(
                cls.query_param),
                                         clear_cache=False)
